...
export default function SeriesForm({ editContent }: SeriesFormProps) {
  ...
  const [statusOptions] = useState<Option[]>([
  ...
  const [formData, setFormData] = useState<SeriesFormData>({
    ...
    numberOfSeasons: editContent?.numberOfSeasons?.toString() || '',
    currentStatus: editContent?.currentStatus || '',
  ...
  const validateForm = (): boolean => {
    ...
    if (!formData.duration || parseFloat(formData.duration) <= 0) {
      newErrors.duration = 'Episode duration must be a positive number';
    }
    ...
    if (!formData.studio.trim()) {
      newErrors.studio = 'Studio/Network is required';
    }
    
    if (!formData.numberOfSeasons || parseInt(formData.numberOfSeasons) <= 0) {
      newErrors.numberOfSeasons = 'Number of seasons must be a positive number';
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const seriesData = {
        ...
        numberOfSeasons: parseInt(formData.numberOfSeasons),
      ...
      console.log('Series data to be sent:', seriesData);
      
      let response;
      if (editContent) {
        // Update existing series
        response = await contentService.updateContent(editContent.id, seriesData);
        console.log('Series updated successfully:', response);
      } else {
        // Create new series
        response = await contentService.createSeries(seriesData);
      ...
      setError(err.response?.data?.message || 'Error creating series. Please try again.');
    } finally {
      setLoading(false);
    }
  };
    ...
    <Box>
      ...
      {success && (
        <Alert severity="success" sx={{ mb: 3 }}>
          {editContent 
            ? 'Series updated successfully! Redirecting to details page...'
            : 'Series created successfully! Redirecting to series page...'
          }
        </Alert>
      )}
      ...
      <form onSubmit={handleSubmit}>
          ...
          <Box sx={{ display: 'flex', gap: 2 }}>
            <Box sx={{ flex: 1 }}>
              <FormField
                ...
                helperText={errors.numberOfSeasons}
                type="number"
                required
              />
            </Box>
        ...
        <Box sx={{ mt: 4, display: 'flex', gap: 2 }}>
          ...
          >
            {loading ? <CircularProgress size={24} sx={{ color: '#FFF' }} /> : (editContent ? 'Update Series' : 'Create Series')}
          </Button>
